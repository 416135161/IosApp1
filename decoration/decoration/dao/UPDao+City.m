...
- (void)createCity:(City *)city{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [self inTransaction:^(FMDatabase *db, BOOL *rollback) {
             ...
             [db executeUpdate:@"INSERT INTO "TABLE_BT_CITY"(cityID,groupName,picName,cdate) VALUES(?,?,?,?)",city.cityID,city.groupName,city.name,[NSDate date]];
...
- (NSMutableArray *)getCitys:(NSString *)cityName{
    ...
    [self inDatabase:^(FMDatabase *db) {
        NSString *sql= [NSString stringWithFormat:@"SELECT * FROM "TABLE_BT_CITY" "];
